...
- **Status**: ✅ done (2026-05-07) — 해결안 B (schema-aware) 채택
...
- **구현**:
  - `_resolve_master_db_schema(notion_api, db_id)` 추가 — properties 한 번 읽고 dict 반환
  - `_find_track_page_in_master_db` schema 파라미터 받아서 `has_track_multi_select` 일 때만 1차 쿼리
  - `_get_or_create_track_page_in_master_db` 도 schema reuse → 트랙당 fetch 횟수 감소
  - `_commit_group_preview_to_notion` 가 시작 시점에 스키마 1회 조회 → 모든 트랙에 재사용
- **효과**:
  - 트랙명 multi_select 없는 DB → 400 에러 노이즈 0
  - 트랙명 multi_select 있는 DB → 기존처럼 1차 쿼리 활용
  - get_database 호출 N회 → 1회로 단축 (트랙 9개 기준 8회 절감)
...
- **구현**:
  - 서버: `_ADMIN_ROLE_CACHE_TTL` 5분 → 30초 (admin_server.py)
  - 클라이언트: `refreshAdminStateFromServer()` 추가 — 페이지 로드 후
    `/api/auth/me` 호출 → `data.isAdmin` 으로 `IS_VIEWER_ADMIN` 갱신
  - 변경 시 헤더 재렌더 (admin 버튼 노출/숨김 즉시 반영)
- **체감**:
  - 디스코드에서 운영자 역할 부여 → 30초 후 새로고침 → 🛡️ 관리자 페이지 버튼 등장
  - 운영자 역할 제거 → 30초 후 새로고침 → 버튼 사라짐
...
### 트랙 파인더 매칭률 순위 역전 (`track-finder.html`)
- **Date**: 2026-07-28
- **Commit**: `ece7bd0` (선행: `67da13c` 빌더 기초/심화 → 단일 통합)
- **증상**: 결과 화면에 `1순위 빌더 80% / 2순위 앱 개발 99%` 처럼 **2순위 매칭률이 더 높게** 표시. 앱 성향 응답이 빌더로 잘못 추천됨
- **원인**: 순위는 원점수(`total`)로 정렬하는데 화면의 매칭률은 정규화 값(`total/max`)이었음. 11기에 빌더가 통합되면서 트랙별 획득 가능 최대점이 벌어져(빌더 30 vs 앱 24) 표면화
- **구현**: `renderResult` 의 정렬 기준을 `total` → `total/max` 비율로 통일. `67da13c` 가 키 순서(app 을 builder 앞에)로 우회하려던 앱 노출 저하도 함께 해소되며, 동점 시 앱 우선이라는 의도는 키 순서로 그대로 보존
- **검증**: node 로 채점 로직 재현 — 6트랙 각각 '노골적 응답' 시 1순위 도달 99%, 매칭률 역전 0건, 무작위 3000회 1순위 분포 491~523 (수정 전 앱은 500회 중 25회로 사실상 미노출)
- **함께 반영**: 트랙 문구·산출물을 `track-apply.html` 의 `PREOPEN_TRACKS` 기준으로 정합(세일즈 → '세일즈 실전 트랙', 앱 = Flutter + 안티그라비티, 나 탐구 = 오프라인 강연 발표 등), 결과 화면에 요일 표기 + 1·2순위가 같은 요일(월: 세일즈·빌더·나 탐구)이면 '둘 중 하나만' 안내
- ⚠️ **강사·파트너·실적 문구는 그대로 뒀음** (조쉬 5만 팔로워 / 2024년 2.7억 / with 마이온 컴퍼니) — 코드에서 검증 불가. 11기 기준으로 갱신이 필요하면 조쉬 확인 후 수정할 것

### 시크릿 백업 파일이 .gitignore 를 통과하던 문제
- **Date**: 2026-07-28
- **증상**: `.env.test.bak` 이 untracked 로 노출. `.gitignore` 가 `.env.test` 는 막지만 사본(`.bak`)은 안 막아 `git add .` 한 번이면 시크릿이 커밋될 수 있었음 (`*.backup` 패턴은 있었으나 `*.bak` 은 없었음)
- **구현**: env 파일을 개별 나열 → 패턴(`.env`, `.env.*`)으로 바꾸고 템플릿만 `!.env.example` 로 예외 처리. `*.bak` / `*.old` 도 백업 섹션에 추가
- **검증**: `git check-ignore` 로 `.env.test.bak` / `.env.prod.bak` / `.env.local.old` / `config.py.bak` 무시 확인, `.env.example` 은 계속 추적됨. 히스토리 전체 스캔 결과 **실제 시크릿이 커밋된 이력은 없음** (추적 중인 env 파일은 `.env.example` 뿐)

### #5. 운영자 역할 변경 즉시 반영 (라이브 isAdmin 갱신)
- **Date**: 2026-05-07
- 위 #5 항목 참고. role 캐시 30초 + 페이지 로드 시 /api/auth/me 동기화.

### #2. Notion 마스터 DB 스키마 미스매치 해소 (schema-aware lookup)
- **Date**: 2026-05-07
- 위 #2 항목 참고. _resolve_master_db_schema 1회 호출 + _find_track_page_in_master_db 가 schema 받아 conditional query.
...
2. ~~#2 Notion 스키마 정리~~ ✅ done
3. ~~#5 isAdmin 갱신 로직~~ ✅ done
4. **#7 gunicorn 전환** ← prod 안정성
5. **#3 commit idempotency** ← 더블클릭/타임아웃 재시도 안전
6. **나머지** 데모 사용 패턴 보면서 우선순위 조정
