...
- **해결안**:
  - POST `/api/mockups/group-preview/commit` → 즉시 `{jobId, status:"running"}` (202)
  - 백그라운드 thread 가 실제 Notion + Discord 작업
  - GET `/api/mockups/group-preview/commit/status?jobId=X` → 진행상황
  - 클라이언트 2초 폴링하며 modal step 갱신 → 완료 시 success 화면
- **관련 파일**: `admin_server.py` (commit endpoint), `static/track-apply.html` (gpFinishStart/gpCallSyncApi)
...
1. **#1 비동기 commit 패턴** ← 데모 안정성 직결, 가장 큰 임팩트
