...
- **운영 명령어** — `!채널삭제 <기수>` (예: `!채널삭제 8` — 해당 기수 트랙 채널·역할 정리, 공지 채널·카테고리·다른 기수 역할은 보존)
...
관리자가 신청 페이지의 `기수·기간 설정` 모달에서 트랙별 요일·세션 시각을 저장할 수 있습니다.

- `trackSchedules`: 트랙 ID/명, 요일(`MON`~`SUN`), 세션 시각, 세션 수, 첫 세션 날짜(선택), 회차별 예외 일정
- `announcementScheduleEnabled`: 공지 일정 연동 활성화 여부
- 첫 세션 날짜를 비우면 운영 공지 봇이 `otDate` 이후 처음 도래하는 설정 요일을 1주차로 계산합니다.
- 기본 공지 규칙은 세션 1일 전 18:00이며 트랙별로 변경할 수 있습니다.
- 같은 회차의 날짜·시간이 바뀌면 `exceptions`로 덮어씁니다.
- 공지 봇은 신청 시작일 이후에만 이 설정을 반영하며, 최종 발송본은 Slack `asc-업무논의` 승인 후 Discord로 발송합니다.

`GET /api/cohort-config`는 공지 봇이 읽는 공개 설정이고, `PUT /api/cohort-config`는 기존과 동일하게 관리자 인증이 필요합니다.
