...
> ## 🚨🚨 제일 먼저 기억할 단 하나 🚨🚨
> **이 봇/서버는 전부 Oracle 서버(`168.107.16.76`)에서 돈다. 이 맥(로컬)에서 안 돈다.**
> → 로컬 코드 수정은 **배포(git push → 서버 git pull) 전까지 라이브에 반영 안 됨.**
> → "왜 반영이 안 되지?" 의 99%는 **미배포** 때문. 배포: `bash scripts/push-and-deploy.sh --light`
